...
import { influencersRoutes } from "../modules/influencers/routes.ts";
...
export const v1Surface: FastifyPluginAsyncTypebox = async (fastify) => {
  ...
  await fastify.register(influencersRoutes, { surface: "v1" });
  await fastify.register(authRoutes, { surface: "v1" });
  await fastify.register(leaguesRoutes, { surface: "v1" });
};
