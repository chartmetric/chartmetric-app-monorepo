...
- Add `.final()` when reading a `ReplacingMergeTree` table, otherwise recently updated rows can appear twice until parts merge. A join target cannot carry `FINAL` — never join such a table directly; read it through a CTE that applies `.final()` and join the CTE. `FINAL` is unnecessary only when the query already aggregates with `argMax` over a version column outside the sorting key, which reproduces it at lower cost.
- Before joining a deduplicated table, compare its sorting key (in `schema.generated.ts`) with the join key. When the join key is not a prefix of the sorting key, `FINAL` cannot collapse duplicates for that join: reduce to one row per join key with `GROUP BY <join key>` and `argMax(column, <version>)`. Verify from the schema, not from row counts — the defect is dormant while the table holds one row per join key.
