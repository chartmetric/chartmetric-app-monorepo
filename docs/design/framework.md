...
**Header composition.** One header row owns the page's controls: title + live count, inline search, then quick-filter pill groups — in that order — with utility controls right-aligned on the same row. It wraps to a balanced second row when it must, never to a lopsided strip. **Concrete acceptance:** at a 1512px viewport, one header row holds title + count + search + every primary pill group + the sort control with margin and the utility cluster right-aligned; nothing wraps, including the header labels themselves (inline `nowrap`). Below that width it wraps by group as defined in [Adaptive layout](#adaptive-layout).

**Default sort is the page's ranking metric,** not the alphabet — which also sinks empty rows to the bottom.

### Adaptive layout

Every layout must stay usable and composed at narrow and mobile widths — the same aesthetic, re-flowed. Narrow behavior is designed, not left to flexbox defaults: the default (`nowrap` containers holding wrapping children) produces exactly the wrong collapse, squeezing each unit until its own atoms stack vertically while the units refuse to reflow.

- **Wrap by group, never inside one.** A header or toolbar is one wrapping flex row whose direct children are its atomic units: the title unit (icon + title + live count), the search field, each labeled pill group, each utility control. Atoms carry `wrap="nowrap"` internally; only the container wraps. An icon, its title, and its count are one piece of information — they move to a new row together or not at all. The only unit allowed to wrap internally is a pill group whose option catalog is unbounded (the sport list), because no row can be guaranteed to fit it.
- **Search is a filter.** It lives in the same wrap flow as the pill groups and reflows with them — never a fixed-width block vertically centered beside a taller wrapped cluster. Like any filter control it is fixed-width from `sm` up and full-width at `base`.
- **Utility controls anchor to the title row when the header wraps.** At full width the utility cluster right-aligns at the end of the single header row (`ml="auto"` inside the wrapping container). Once the viewport forces wrapping, a trailing cluster would land alone under the filters — the lopsided strip rule above prohibits exactly that — so below the width where one row fits, it moves next to the title and shares that row (a `hiddenFrom`/`visibleFrom` pair at the matching breakpoint, plus a zero-height `flex="1 0 100%"` break item that closes the title row so the filters start beneath it; `display: none` keeps the inactive copy out of the accessibility tree, and keyboard order matches visual order in both modes, which a CSS `order` swap would break). Pick that breakpoint **at or above** the width where the single row genuinely fits: set below it, there is an in-between range where the cluster first wraps to the bottom row and then jumps to the title row as the viewport narrows — exactly the artifact this rule exists to prevent.
- **Fixed dimensions are for `sm` and up.** Any fixed control width is written as a responsive prop (`w={{ base: "100%", sm: N }}`). Wide content (tables, charts) scrolls inside its own container; the page body never scrolls horizontally.
- **Verify narrow states before calling a view done.** 1512px is the reference width; also check a tablet width (~768px) and a phone width (~390px), in both color schemes (`method.md`'s screenshot tooling takes a width argument).
