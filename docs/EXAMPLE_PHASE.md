...
- `smoke_cmd` is null. The phase adds no route and wires nothing new,
  so there is no orchestration to exercise. It still changes a query,
  and a query change is exactly where stubbed builders and asserted SQL
  strings pass while ClickHouse rejects the statement (ambiguous
  identifiers, ambiguous join keys). The unit tests prove the builder
  emits what we expect; only execution proves ClickHouse accepts it —
  so run the matrix through the read-only ClickHouse MCP and record the
  results in `notes`, rather than committing a runner to do it. Cover
  each filter, each sort, the count/list sibling pair, empty values,
  and include/exclude modes.
